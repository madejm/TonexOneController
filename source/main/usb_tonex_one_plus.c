...
//0x06   0x0409  "TONEX ONE+ Control VCOM"
//0x07   0x0409  "TONEX ONE+ MIDI Control"
//0x08   0x0409  "TONEX ONE+ MIDI Streaming"
...
//0x12   0x0409  "TONEX ONE+ MIDI In"
//0x13   0x0409  "TONEX ONE+ MIDI Out"
//0x14   0x0409  "xxxxxxxxxxxxxxxxxxxx"     // Serial Number
//----------------------------------------------------
...
static const char *TAG = "app_TonexOnePlus";

// preset name is proceeded by this byte sequence:
static const uint8_t ToneOnePlusPresetByteMarker[] = {0xB9, 0x04, 0xB9, 0x02, 0xBC, 0x21};

// preset index is proceeded by this byte sequence:
static const uint8_t ToneOnePlusPresetIndexMarker[] = {0xB9, 0x06, 0x01};
...
*/
static TonexStatus usb_tonex_one_plus_parse(uint8_t* message, uint16_t inlength);
static esp_err_t usb_tonex_one_plus_set_active_slot(Slot newSlot);
static esp_err_t usb_tonex_one_plus_set_preset_in_slot(uint16_t preset, Slot newSlot, uint8_t selectSlot);
static esp_err_t usb_tonex_one_plus_set_ab_slots(uint16_t preset_a, uint16_t preset_b);
static uint16_t usb_tonex_one_plus_get_current_active_preset(void);
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_hello(void)
{
    uint16_t outlength;
    
    ESP_LOGI(TAG, "Sending Hello");

    uint8_t request[] = {0xb9, 0x03, 0x00, 0x82, 0x04, 0x00, 0x80, 0x15, 0x01, 0xb9, 0x02, 0x02, 0x16};
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_request_state(void)
{
    uint16_t outlength;

    uint8_t request[] = {0xb9, 0x03, 0x00, 0x82, 0x06, 0x00, 0x80, 0x15, 0x03, 0xb9, 0x02, 0x81, 0x04, 0x03, 0x16};
...
* NOTES:       
*****************************************************************************/
static esp_err_t __attribute__((unused)) usb_tonex_one_plus_request_preset_details(uint8_t preset_index, uint8_t full_details)
{
    uint16_t outlength;

    ESP_LOGI(TAG, "Requesting full preset details for %d", (int)preset_index);

    uint8_t request[] = {0xb9, 0x03, 0x81, 0x00, 0x03, 0x82, 0x06, 0x00, 0x80, 0x15, 0x03, 0xb9, 0x04, 0x16, 0x01, 0x00,  0x00};  
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_send_single_parameter(uint16_t index, float value)
{
    uint16_t framed_length;

    // Build message                                         len LSB  len MSB
    uint8_t message[] = {0xb9, 0x03, 0x81, 0x06, 0x03, 0x82, 0x0A,     0x00, 0x80, 0x15, 0x03};
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_send_master_volume(float value)
{
    uint16_t framed_length;
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_request_master_volume(void)
{
    uint16_t framed_length;
...
* NOTES:       
*****************************************************************************/
static void __attribute__((unused)) usb_tonex_one_plus_dump_state(void)
{
    float InputTrim;
    float BPM;
    uint16_t TuningRef;
...
* NOTES:       
*****************************************************************************/
static esp_err_t __attribute__((unused)) usb_tonex_one_plus_set_active_slot(Slot newSlot)
{
    uint16_t framed_length;
...
static esp_err_t usb_tonex_one_plus_set_preset_in_slot(uint16_t preset, Slot newSlot, uint8_t selectSlot)
{
    ...
    uint8_t message[] = {0xb9, 0x03, 0x81, 0x04, 0x03, 0x82, 0,       0,       0x80, 0x15, 0x03};
    ...
    switch (control_get_config_item_int(CONFIG_ITEM_SAVE_PRESET_TO_SLOT))
    {
        ...
        //todo here: current slot is set by different message usb_tonex_one_plus_set_active_slot
    ...
    {
        if (selectSlot && (TonexData->Message.CurrentSlot == newSlot) && (preset == usb_tonex_one_plus_get_current_active_preset()))
        {
    ...
    {
        // modify the buffer with the new slot
        //todo, needs another message send usb_tonex_one_plus_set_active_slot
    }
...
static esp_err_t usb_tonex_one_plus_set_ab_slots(uint16_t preset_a, uint16_t preset_b)
{
    ...
    return usb_tonex_one_plus_request_preset_details(preset_a, 0);
}
...
* NOTES:       
*****************************************************************************/
static bool usb_tonex_one_plus_handle_rx(const uint8_t* data, size_t data_len, void* arg)
{
    // debug
    ...
    {
        ESP_LOGE(TAG, "usb_tonex_one_plus_handle_rx data too long! %d", (int)data_len);
        return false;
    }
    else
    {
    ...
    ESP_LOGE(TAG, "usb_tonex_one_plus_handle_rx no available buffers!");
    return false;
}
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_modify_global(uint16_t global_val, float value)
{
...
* NOTES:       
*****************************************************************************/
static TonexStatus usb_tonex_one_plus_parse_state(uint8_t* unframed, uint16_t length, uint16_t index)
{
    ...
    if (tonex_params_colors_get_locked_access(&color_ptr) == ESP_OK)
    {
        ...
        for (uint16_t preset_index = 0; preset_index < MAX_PRESETS_TONEX_ONE_PLUS; preset_index++) {
...
* NOTES:       
*****************************************************************************/
static TonexStatus usb_tonex_one_plus_parse_current_preset(uint8_t* unframed, uint16_t length, uint16_t index)
{
    tModellerParameter* param_ptr;
    ...
    usb_tonex_one_plus_request_preset_details(TonexData->Message.CurrentPreset, 0);
...
* NOTES:       
*****************************************************************************/
static TonexStatus usb_tonex_one_plus_parse_param_changed(uint8_t* unframed, uint16_t length, uint16_t index)
{
    ...
    if (temp_ptr != NULL)
    {
        ...
        {
            ESP_LOGW(TAG, "usb_tonex_one_plus_parse_param_changed 0x88 not found");
        }
    }
    else
    {
        ESP_LOGW(TAG, "usb_tonex_one_plus_parse_param_changed start marker not found");
        ESP_LOG_BUFFER_HEXDUMP(TAG, unframed, length, ESP_LOG_INFO);
    }
...
* NOTES:       
*****************************************************************************/
static Slot usb_tonex_one_plus_slot_for_saving_preset(void)
{
    Slot slot = C;
    switch (control_get_config_item_int(CONFIG_ITEM_SAVE_PRESET_TO_SLOT))
    {
...
* NOTES:       
*****************************************************************************/
static void usb_tonex_one_plus_parse_preset_parameters(uint8_t* raw_data, uint16_t length)
{
...
static TonexStatus usb_tonex_one_plus_parse(uint8_t* message, uint16_t inlength)
{
    ...
    ESP_LOGI(TAG, "usb_tonex_one_plus_parse: type: %d size: %d", (int)header.type, (int)header.size);
    ...
    switch (header.type)
    {
        ...
        {
            return usb_tonex_one_plus_parse_state(FramedBuffer, out_len, index);
        } break;
        
        case TYPE_STATE_PRESET_DETAILS:
        {
            return usb_tonex_one_plus_parse_preset_details(FramedBuffer, out_len, index);
        } break;
        ...
        {
            return usb_tonex_one_plus_parse_param_changed(FramedBuffer, out_len, index);
        } break;
...
* NOTES:       
*****************************************************************************/
static esp_err_t usb_tonex_one_plus_process_single_message(uint8_t* data, uint16_t length)
{
    void* temp_ptr;  
    ...
    if ((length >= 2) && (data[0] == 0x7E) && (data[length - 1] == 0x7E))
    {
        ...
        TonexStatus status = usb_tonex_one_plus_parse(data, length);
        ...
        else
        {
            ...
            switch (TonexData->Message.Header.type)
            {
                case TYPE_STATE_UPDATE:
                {
                    current_preset = usb_tonex_one_plus_get_current_active_preset();
                    ...
                    if (boot_init_needed)
                    {
                        ...
                        usb_tonex_one_plus_request_preset_details(boot_preset_request, 0);
                        boot_init_needed = 0;
                    }
                    else
                    {
                ...
                case TYPE_STATE_PRESET_DETAILS:
                {
                    // locate the ToneOnePlusPresetByteMarker[] to get preset name
                    temp_ptr = memmem((void*)data, length, (void*)ToneOnePlusPresetByteMarker, sizeof(ToneOnePlusPresetByteMarker));
                    if (temp_ptr != NULL)
                    {
                        ESP_LOGI(TAG, "Got preset name");

                        // grab name
                        memcpy((void*)preset_name, (void*)(temp_ptr + sizeof(ToneOnePlusPresetByteMarker)), TONEX_ONE_RESP_OFFSET_PRESET_NAME_LEN);                
                    }

                    current_preset = usb_tonex_one_plus_get_current_active_preset();

                    if (boot_preset_request < MAX_PRESETS_TONEX_ONE_PLUS) 
                    {
                        // find out which preset index this is, so we can't get out of sync
                        temp_ptr = memmem((void*)data, length, (void*)ToneOnePlusPresetIndexMarker, sizeof(ToneOnePlusPresetIndexMarker));
                        ...
                        {
                            // found index
                            memcpy((void*)&preset_index, (void*)(temp_ptr + sizeof(ToneOnePlusPresetIndexMarker)), sizeof(preset_index));                            
                        }
                        else
                        {
                    ...
                    else if (boot_preset_request == MAX_PRESETS_TONEX_ONE_PLUS) 
                    {
                        // all other preset nammes grabbed, get current preset details
                        boot_preset_request++;
                        usb_tonex_one_plus_request_preset_details(current_preset, 0);
                    } 
                    else 
                    {
                        ESP_LOGI(TAG, "Received State Update. Current slot: %d. Preset: %d", (int)TonexData->Message.CurrentSlot, (int)current_preset);
                        ...
                        usb_tonex_one_plus_parse_preset_parameters(data, length);
                ...
                case TYPE_HELLO:
                {
                    ...
                    // get current state
                    usb_tonex_one_plus_request_state();
...
{        
    ...
    switch (TonexData->TonexState)
    {
        ...
        {
            // do the hello 
            if (usb_tonex_one_plus_hello() == ESP_OK)
            {
            ...
            if (xQueueReceive(input_queue, (void*)&message, 0) == pdPASS)
            {
                ...
                switch (message.Command)
                {
                    case USB_COMMAND_SET_PRESET:
                    {
                        ...
                        {
                            if (usb_tonex_one_plus_set_preset_in_slot(message.Payload, usb_tonex_one_plus_slot_for_saving_preset(), 1) != ESP_OK)
                            {
                    ...
                    case USB_COMMAND_LOAD_PRESET_TO_SLOT_A:
                    {
                        if (message.Payload < MAX_PRESETS_TONEX_ONE_PLUS)
                        {
                            ...
                            if (usb_tonex_one_plus_set_preset_in_slot(message.Payload, A, 0) != ESP_OK)
                            {
                        ...
                        {
                            ESP_LOGW(TAG, "Invalid preset index %d for Slot A (max %d)", (int)message.Payload, MAX_PRESETS_TONEX_ONE_PLUS - 1);
                        }
                    } break;
                    ...
                    case USB_COMMAND_LOAD_PRESET_TO_SLOT_B:
                    {
                        if (message.Payload < MAX_PRESETS_TONEX_ONE_PLUS)
                        {
                            ...
                            if (usb_tonex_one_plus_set_preset_in_slot(message.Payload, B, 0) != ESP_OK)
                            {
                        ...
                        {
                            ESP_LOGW(TAG, "Invalid preset index %d for Slot B (max %d)", (int)message.Payload, MAX_PRESETS_TONEX_ONE_PLUS - 1);
                        }
                    } break;   
                    ...
                    case USB_COMMAND_SET_AB_SLOTS:
                    {
                        ...
                        if ((preset_a < MAX_PRESETS_TONEX_ONE_PLUS) && (preset_b < MAX_PRESETS_TONEX_ONE_PLUS))
                        {
                            ESP_LOGI(TAG, "Set AB slots via MIDI. A: %d B: %d", (int)preset_a, (int)preset_b);
                            if (usb_tonex_one_plus_set_ab_slots(preset_a, preset_b) != ESP_OK)
                            {
                        ...
                        {
                            ESP_LOGW(TAG, "Invalid AB slot presets A:%d B:%d (max %d)", (int)preset_a, (int)preset_b, MAX_PRESETS_TONEX_ONE_PLUS - 1);
                        }
                    } break;
                    ...
                    case USB_COMMAND_MODIFY_PARAMETER:
                    {
                        if (message.Payload < TONEX_PARAM_LAST)
                        {
                            ...
                            // send it
                            usb_tonex_one_plus_send_single_parameter(message.Payload, message.PayloadFloat);
                        }
                        else if (message.Payload < TONEX_GLOBAL_LAST)
                        {
                            // modify the global
                            if (usb_tonex_one_plus_modify_global(message.Payload, message.PayloadFloat) == ESP_OK)
                            {
                                // debug
                                //usb_tonex_one_plus_dump_state(&TonexData->Message.PedalData.TonexStateData);

                                // send it by setting the same preset active again, which sends the state data
                                usb_tonex_one_plus_set_preset_in_slot(usb_tonex_one_plus_get_current_active_preset(), usb_tonex_one_plus_slot_for_saving_preset(), 1);

                                // set slot active
                                usb_tonex_one_plus_set_active_slot(usb_tonex_one_plus_slot_for_saving_preset());
                            }
                        }
                        else
                        {
    ...
    for (uint8_t loop = 0; loop < MAX_INPUT_BUFFERS; loop++)
    {
        if (InputBuffers[loop].ReadyToRead)
        {
            ...
            {    
                ...
                // process it
                if (usb_tonex_one_plus_process_single_message(rx_entry_ptr, end_marker_pos + 1) != ESP_OK)
                {
                    break;    
                }
...
void usb_tonex_one_plus_init(class_driver_t* driver_obj, QueueHandle_t comms_queue)
{
    ...
    ESP_LOGI(TAG, "Opening CDC ACM device 0x%04X:0x%04X", IK_MULTIMEDIA_USB_VENDOR, TONEX_ONE_PLUS_PRODUCT_ID);
    ...
    const cdc_acm_host_device_config_t dev_config = {
        ...
        .user_arg = NULL,
        .event_cb = NULL,
        .data_cb = usb_tonex_one_plus_handle_rx
    };
    ...
    // open it
    ESP_ERROR_CHECK(cdc_acm_host_open(IK_MULTIMEDIA_USB_VENDOR, TONEX_ONE_PLUS_PRODUCT_ID, TONEX_ONE_CDC_INTERFACE_INDEX, &dev_config, &cdc_dev));
    assert(cdc_dev);
...
* NOTES:       
*****************************************************************************/
void usb_tonex_one_plus_deinit(void)
{
    // close USB
